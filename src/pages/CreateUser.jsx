...
const CreateUser = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // 3. Create the profile immediately using the admin (logged-in) supabase client
      // This bypasses email confirmation so the user can log in right away
      if (signUpData?.user?.id) {
        ...
        if (profileError) {
          ...
          // Don't throw - auth user was created, profile can be re-created on first login
        }
      }
      ...
      const rawMessage = err?.message || 'An unexpected error occurred. Please try again.'
      const normalized = rawMessage.toLowerCase()

      if (normalized.includes('rate limit')) {
        setError('Email service is temporarily rate-limited. Please wait a few minutes and try again.')
      } else if (normalized.includes('already exists') || normalized.includes('duplicate') || normalized.includes('unique')) {
